...
//-- Martin Webrant
  ...
  gmsgFpsWarning  = REG_USER_MSG("FpsWarning", 1);  //Warning when a player exceeds the fps limit
  gmsgFpsKick     = REG_USER_MSG("FpsKick", 1);     //When a player needs to be kicked for exceeding the fps limit
  gmsgFovKick	  = REG_USER_MSG("FovKick", 1);     //When a player needs to be kicked for exceeding the fov limit
  //-- Martin Webrant
...
void CBasePlayer::LimitFps()
{
	...
	{
		// I'll leave the job of logging and kicking the player to the amxx plugin because i want to have multilang support
